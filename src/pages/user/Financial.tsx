    ...
    if (assignments) {
      ...
      assignments.forEach((a) => {
        if (a.checkin_at && a.checkout_at) {
          const checkin = new Date(a.checkin_at);
          const checkout = new Date(a.checkout_at);
      ...
      setSummary({
        ...
        totalHours,
        totalValue: assignments.reduce((sum, a) => sum + Number(a.assigned_value || 0), 0),
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Seu resumo financeiro mensal</p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold">R$ {summary.totalValue.toFixed(2)}</span>
              {summary.status && (
                <Badge className={statusColors[summary.status as keyof typeof statusColors]} variant="outline">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Plantões Concluídos</CardTitle>
        </CardHeader>
              ...
              {shifts.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground">
                    Nenhum plantão concluído no período
                  </TableCell>
                </TableRow>
              ) : (
                shifts.map((s) => (
                  <TableRow key={s.id}>
                    <TableCell>
                      {new Date(s.shift.shift_date).toLocaleDateString('pt-BR')}
                    </TableCell>
                    <TableCell>
                      <div>
                        <p className="font-medium">{s.shift.title}</p>
                        <p className="text-sm text-muted-foreground">{s.shift.hospital}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {s.checkout_at ? new Date(s.checkout_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '-'}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      R$ {Number(s.assigned_value).toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))
              )}
